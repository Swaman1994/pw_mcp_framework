...
test.describe('SauceDemo checkout overview', () => {
  test('displays checkout totals and finish action @smoke @critical', async ({ page }, testInfo) => {
    const overview = await test.step('Complete checkout information', async () => {
      ...
      await attachStepScreenshot(page, testInfo, 'Complete checkout information');
      return overview;
    });
    await test.step('Verify checkout totals and finish action', async () => {
      ...
      await attachStepScreenshot(page, testInfo, 'Verify checkout totals and finish action');
    });
  });
});